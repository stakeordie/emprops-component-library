...
main() {
    ...
    log "12. Setup Redis Worker Repository"
    log "13. Start Redis Worker Services"
    log "14. Verify all services"
    log "====================================="
    log ""
    ...
    # Phase 12: Setup Redis Worker Repository
    log_phase "12" "Setting up Redis Worker Repository"
    setup_redis_worker
    
    # Phase 13: Start Redis Worker Services
    log_phase "13" "Starting Redis Worker Services"
    start_redis_worker
...
verify_and_report() {
    ...
    if [ "$all_services_ok" = true ]; then
        ...
        log "Use 'mgpu logs-all' to monitor ComfyUI services"
        log "Use 'wgpu logs all' to monitor worker services"
        log "===================================="
        return 0
    else
        log ""
...
verify_services() {
    ...
    # 3. Check Redis Worker Services
    log "Checking Redis Worker services..."
    if ! wgpu status all >/dev/null 2>&1; then
        log "ERROR: Redis Worker services are not running properly"
        wgpu status all 2>&1 | while read -r line; do log "  $line"; done
        all_services_ok=false
    else
        log "Redis Worker services are running"
    fi
...
setup_redis_worker() {
    ...
    if [ ! -d "${ROOT}/redis-worker" ]; then
        ...
        log "Redis worker repository cloned successfully"
    else
        log "Redis worker repository already exists"
    fi
    
    # Verify wgpu script is available
    if ! command -v wgpu >/dev/null 2>&1; then
        log "ERROR: WGPU command not found in PATH"
        return 1
    fi
    
    # Setup worker directories
    log "Setting up worker directories..."
    if ! wgpu setup all; then
        log "ERROR: Failed to set up worker directories"
        return 1
    fi
    
    log "Redis worker setup complete"
    return 0
}

start_redis_worker() {
    log "Starting Redis Worker Services..."
    
    # Start worker instances
    if ! wgpu start all; then
        log "ERROR: Failed to start Redis worker services"
        wgpu status all 2>&1 | tail -n 5 | while read -r line; do log "  $line"; done
        return 1
    fi
    
    # Check worker status
    log "Checking worker status..."
    if ! wgpu status all >/dev/null; then
        log "ERROR: Worker status check failed"
        return 1
    fi
    
    log "Redis worker services started successfully"
    return 0
}
